(function (root, factory) {
   ...
   /* :-[ / :-@  */ rage: /:-?[\[@]/g,
         ...
         /* :+1: */ thumbsup: /:\+1:/g,
         /* :-1: */ thumbsdown: /:\-1:/g,
         /* :n */ white_small_square: /:n/g,
         /* :y */ black_small_square: /:y/g,
         /* :y */ 'nothing': /:nothing/g,
                };
            ...
            function insertEmojicon(args) {
                ...
                if(args.replacer){
                    ...
                    var name = args.emojiName;
                    var comment_start = name.indexOf("?");
                    var comments = "";
                    if (comment_start != -1){
                        comments = name.substr(comment_start+1);
                        name = name.substr(0, comment_start);
                    }
                    if (name.startsWith("fa-") || name.startsWith("fas-") || name.startsWith("far-") || name.startsWith("fab-")){
                        emojiElement = args.win.document.createElement('i');
                        var dashp = name.indexOf('-');
                        emojiElement.setAttribute('class', name.substr(0,dashp) + ' ' + 'fa-'+name.substr(dashp+1));
                        emojiElement.setAttribute('alt', comments);
                        emojiElement.setAttribute('style', 'font-size:18px; margin-bottom:1px; display:inline-flex');
                        
                    }
                    else if (name.startsWith("ma-")){
                        emojiElement = args.win.document.createElement('i');
                        emojiElement.setAttribute('class', 'small material-icons');
                        emojiElement.setAttribute('alt', comments);
                        emojiElement.innerHTML=name.substr(3);
                        emojiElement.setAttribute('style', 'font-size:24px; display: inline-flex;  vertical-align: bottom;');
                    }
                        ...
                        emojiElement.setAttribute('class', 'emoji emoji-' + name);
                        emojiElement.setAttribute('style', 'height:24px; width:24px;margin-bottom:-4px; display:inline-block;');
                    } else {
                        emojiElement = args.win.document.createElement('img');
                        emojiElement.setAttribute('alt', ':' + name + ':');
                        emojiElement.setAttribute('class', 'emoji lozad');
                        emojiElement.setAttribute('data-src', defaultConfig.img_dir + '/' + name + '.png');
                        emojiElement.setAttribute('style', 'height:24px; width:24px;margin-bottom:-4px; display:inline-block;');
                    }

                    emojiElement.setAttribute('title', comments);
                }
            ...
            function getEmojiNameForMatch(match) {
                ...
                if(match[1] && match[2]) {
                    ...
                    var to_check = named;
                    if (named.indexOf('?')!=-1){
                        to_check = named.substr(0, named.indexOf('?'));
                    }
                    if(namedMatchHash[to_check] || to_check.startsWith("fa-") || to_check.startsWith("far-") || to_check.startsWith("fas-") || to_check.startsWith("fab-") || to_check.startsWith("ma-") || to_check.startsWith("my-")) { return named; }
                    return;
                }
            ...
            function defaultReplacer(emoji, name) {
                ...
                if (name.startsWith("fa-")){
                    return "<i class='fa " + name + "'>"+comments+"</i>";
                }
                else if (name.startsWith("ma-")){
                    return "<i class='material-icons' title='"+ comments +"'>" + name.substr(3) + "</i>";
                }
                else if (elementType !== 'img') {
                    return "<" +  elementType +" class='emoji emoji-" + name + "' title='" + comments + "'></" + elementType+ ">";
                } else {
                    return "<img align='absmiddle' alt='" + comments + "' class='emoji' src='" + this.config.img_dir + '/' + name + ".png' title='" + comments + "' />";
                }
            }
